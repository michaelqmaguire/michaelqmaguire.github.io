...
I went ahead and grabbed some local medical institutions that presumable have prescribing capabilities. 

I figured that something like this is relateable considering we live in the area.
...
If you do not have these packages installed, do the following in your console:
...
I'm going to store the addresses for the pharmacies into an object called 'pharmacies'.
...
I like to think of it like you're feeding the object to whatever follows it. I like to eat, so maybe think of the pipe as you taking your favorite food (the object) and feeding it into your body (whatever follows).
...
So here, we're doing a couple things:
  ...
  7. Let's keep that feeding going -- we're taking that chewed-up pharmacies object from #2-#6 and feeding it into another object.
...
Here is what I would suggest for you to do:
  ...
  1. Create the (1) Address, (2) City, (3) State, and (4) Zip fields for each address that you need.
  2. Load/install `tidyverse`, `tidygeocoder`, and `readxl` packages. You do not need to reinstall if you've already installed one.
  2. Read in the dataset into R using:
      - `read_excel` from the `readxl` package for Excel files. **MUST INSTALL AND LOAD!**
      - `read_csv` from the `readr` package (automatically loaded with `tidyverse`)
      - `read.csv` using the base-R `utils` package (you do not need to load this - it is already loaded when you start R)
  3. Feed your imported dataset `tidygeocoder`.
  4. Output the file.
